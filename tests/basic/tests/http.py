...
class HTTPTestCase(TestServerTestCase):
    ...
    def test_limit_xss(self):
        """
        Ensure that html cannot be injected in a wrong limit parameter
        """
        connection = self.get_connection()
        connection.request('GET', '/api/v1/notes/?limit=<img%20src="http://ycombinator.com/images/y18.gif">', headers={'Accept': 'application/json'})
        response = connection.getresponse()
        connection.close()
        self.assertEqual(response.status, 400)
        self.assertTrue("<" not in response.read())
